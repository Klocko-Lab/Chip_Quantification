...
# Process K36 track into 1 (enriched) or 0 (not enriched)
'''Will need to specify our ChIPk36_path'''
k36_path = 'set-2_H3K36me2_ChIPseq_nc_14_10kb_for_k36-k9_test.txt'
k36_array = np.transpose(np.loadtxt(k36_path))
k36_median = np.median(k36_array)
k36_median_plus = k36_median + (k36_median * 1)
for i in range(0, k36_array.size):
	if k36_array[i] < k36_median_plus:
		k36_array[i] = 0
	else:
		k36_array[i] = 1
...
for i in range(0, array_y_dim):
	...
	for j in range(0, array_x_dim):
		...
		# determine if region passes thresholds, Thresholds are altered from histone to histone?
			...
			# Commented out since we arent interested in inter links
			...
			if(value >= 2.25 and k9_array[j] == 1 and lg_string1 == lg_string2):
				...
				intra_links_k362k9 = intra_links_k362k9 + 1
...
print('Intra links K36 to K9: ' + str(intra_links_k362k9))
print('Intra links K36 to not K9: ' + str(total_intra_links - intra_links_k362k9))
